...
//		String sql = MessageFormat.format("select {0} AS {1},{2} AS {3} from {4}", APPIC, APPIN, APPCC, APPCN, APPTN);
		String sql = MessageFormat.format("select {0} AS {1},{2} AS {3} from {4}", APPIC, VxApiDATAStoreConstant.AS_APPLICATION_ID_NAME, APPCC, VxApiDATAStoreConstant.AS_APPLICATION_CONTENT_NAME, APPTN);
...
//			String sql = MessageFormat.format("select {0} AS {1} ,{2} AS {3} from {4} where {0} = ? ", APPIC, APPIN, APPCC, APPCN, APPTN);
			String sql = MessageFormat.format("select {0} AS {1} ,{2} AS {3} from {4} where {0} = ? ", APPIC, VxApiDATAStoreConstant.AS_APPLICATION_ID_NAME, APPCC, VxApiDATAStoreConstant.AS_APPLICATION_CONTENT_NAME, APPTN);
...
//			String sql = MessageFormat.format("select {0} AS {1},{2} AS {3},{4} AS {5} from {6} where {2} = ? ", APIIC, APIIN, API_APPIC,API_APPIN, APICC, APICN, APITN);
			String sql = MessageFormat.format("select {0} AS {1},{2} AS {3},{4} AS {5} from {6} where {2} = ? ", APIIC, VxApiDATAStoreConstant.AS_API_ID_NAME, API_APPIC,VxApiDATAStoreConstant.AS_API_APP_ID_NAME, APICC, VxApiDATAStoreConstant.AS_API_CONTENT_NAME, APITN);
...
//			String sql = MessageFormat.format("select {0} AS {1},{2} AS {3},{4} AS {5} from {6} where {0} = ? ", APIIC, APIIN, API_APPIC,API_APPIN, APICC, APICN, APITN);
			String sql = MessageFormat.format("select {0} AS {1},{2} AS {3},{4} AS {5} from {6} where {0} = ? ", APIIC, VxApiDATAStoreConstant.AS_API_ID_NAME, API_APPIC,VxApiDATAStoreConstant.AS_API_APP_ID_NAME, APICC, VxApiDATAStoreConstant.AS_API_CONTENT_NAME, APITN);
			JsonArray params = new JsonArray();
			params.add(name);
			jdbcClient.queryWithParams(sql, params, res -> {
				if (res.succeeded()) {
	...
	public void findBlacklist(Message<JsonObject> msg) {
		String sql = MessageFormat.format("select {0} AS {1} ,{2} AS {3} from {4} ", BLIC, VxApiDATAStoreConstant.AS_BLACKLIST_ID_NAME, BLCC, VxApiDATAStoreConstant.AS_BLACKLIST_CONTENT_NAME, BLTN);
		jdbcClient.query(sql, res -> {
			if (res.succeeded()) {
				List<JsonObject> rows = res.result().getRows();
				JsonObject result = null;
...
//			String sql = MessageFormat.format("REPLACE INTO {0} ({1},{2}) values(?,?)", BLTN, BLIC, BLCC);
			String sql = MessageFormat.format("MERGE INTO {0} T1 " + 
					                          "USING (SELECT ? AS a,? AS b FROM dual) T2 " + 
					                          "ON ( T1.{1}=T2.a) " + 
					                          "WHEN MATCHED THEN " + 
					                          "UPDATE SET T1.{2} = T2.b " + 
					                          "WHEN NOT MATCHED THEN " + 
					                          "INSERT ({1},{2}) VALUES(T2.a,T2.b)", BLTN, BLIC, BLCC);
			JsonArray params = new JsonArray();
